...
### Постоянная база: Neon PostgreSQL

Без внешней базы сайт работает в демо-режиме: `deploy/demo.sqlite3` копируется в `/tmp`, изменения **временные**.
Чтобы данные сохранялись, подключите Neon:

1. Vercel → проект → **Storage** → **Create Database** → **Neon** (Serverless Postgres), регион **Frankfurt (fra1)**, бесплатный план.
2. **Connect** к проекту `next-generation-hub` (окружения Production и Preview). Vercel сам добавит переменные `DATABASE_URL`, `DATABASE_URL_UNPOOLED` и др.
3. **Redeploy** (или любой `git push`).

При первом запуске сайт сам создаст таблицы и заполнит пустую базу демо-данными (`core/bootstrap.py`).
При следующих деплоях новые миграции тоже применяются автоматически (выключить: `DJANGO_AUTO_MIGRATE=False`).
Строку подключения можно использовать и локально: `DATABASE_URL=postgres://… python manage.py …`.

Остальные ограничения Vercel:
- загрузка видеофайлов до 4 МБ (лимит на тело запроса), загруженные файлы пока временные — для них нужно отдельное хранилище (например Vercel Blob). Ссылки на YouTube работают без ограничений;
- демо-логины показываются на странице входа (`SHOW_DEMO_ACCOUNTS=False` — скрыть). После подключения Neon смените пароли демо-аккаунтов.
